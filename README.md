...
- **Multi-select with cut/copy/paste/move** — long-press to select multiple files/folders, then act on them from a bottom action bar (Cut, Copy, Move to…, Share, Delete, and a "More" menu for Rename/Zip/Unzip/Save to device — Google Files-style). A persistent clipboard bar appears whenever something's cut or copied, so paste is never hidden behind a menu.
...
- **Save to device** — save selected files (or whatever's open in the media viewer) straight to `Downloads/PartySync` on the phone, streamed directly from the server.
